 ...
 * to [n], with processing time cut in half by only allocating mask memory to
 * odd numbers and by only looping through multiples of odd numbers.
 */
...
TEST_CASE("test primeNumbers with small N") {
    unsigned long n {30};
...
TEST_CASE("test primeNumbers with large N") {
    unsigned long n {10'000};
    int expectedSize {1229};
    std::vector<unsigned long> expectedTail { 9887, 9901, 9907, 9923, 9929,
