...
//Repeatedly calculates the lcm of 2 values (via reduce()), starting from the
// largest and stepping backwards until the middle of the range, as the
// smaller half of a range will already be factors of the larger half.
unsigned long long lcmOfRange(unsigned short n)
{
    unsigned short limit = (n & 1) == 0 ? n / 2 : n / 2 + 1;
    std::vector<unsigned short> range(limit);
    // initialise a range of decrementing values
    iota(range.rbegin(), range.rend(), n / 2 + 1);
    return accumulate(
            range.rbegin(),
            range.rend(),
...
unsigned long long lcmOfRangeUsingPrimes(unsigned short n)
{
    ...
    std::vector<unsigned long> primes = primeNumbers(n);
    for (unsigned long prime : primes) {
        if (prime * prime > n) {
            result *= prime;
        }
        else {
            auto exponent = static_cast<unsigned long>(log2(n) / log2(prime));
            result *= static_cast<unsigned long long>(pow(prime, exponent));
        }
    }
    return result;
}
...
TEST_CASE("test lower constraints") {
    ...
    std::vector<unsigned long long> expected { 1, 2, 6 };

    for (int i = 0; i < nValues.size(); i++) {
        CHECK_EQ(expected[i], lcmOfRange(nValues[i]));
        CHECK_EQ(expected[i], lcmOfRangeUsingPrimes(nValues[i]));
    }
}

TEST_CASE("test mid constraints") {
    std::vector<unsigned short> nValues { 6, 10, 20 };
    std::vector<unsigned long long> expected { 60, 2520, 232'792'560 };
...
TEST_CASE("test upper constraints") {
    std::vector<unsigned short> nValues { 30, 40 };
    std::vector<unsigned long long> expected { 2'329'089'562'800,
