...
unsigned long collatzLength(unsigned long start)
{
    ...
    while (start != 1) {
        ...
        if (start && !(start & start - 1)) {
            count += static_cast<unsigned long>(std::log2(start)) + 1uL;
            break;
        } else {
            count++;
        }
...
unsigned long collatzLengthMemo(unsigned long start)
{
    ...
    unsigned long count {1uL};
    if ((start & 1) > 0) {  // if x AND 1 > 0 then x is odd
...
unsigned long* generateLongestStarters()
{
    ...
    longestCountedUnderN[0] = 1uL;

    unsigned long longestStarter {1uL}, longestCount {1uL};
